...
REGULAR_CLIENTS = [
    ...
    "Baxrom 9703", "Bahodir akaЁЯЪЫ", "Bahodir aka Andijon", "Akrom aka", 
    "Zo╩╗r mebel", "Umid", "Akmal aka", "Do╩╗kon 707", "Farxod Jomiy", "Munosib Mebel"
]
...
def main_menu(role):
    ...
    if role == 'admin':
        buttons = [
            [types.KeyboardButton(text="ЁЯУж Mavjud mebellar"), types.KeyboardButton(text="ЁЯУЭ Yangi buyurtma")],
            [types.KeyboardButton(text="ЁЯУЛ Buyurtmalar nazorati"), types.KeyboardButton(text="ЁЯУК Mijozlar hisoboti")],
            [types.KeyboardButton(text="ЁЯЪЪ Haydovchilar hisoboti"), types.KeyboardButton(text="ЁЯХ░ Yetkazish tarixi")],
            [types.KeyboardButton(text="ЁЯУИ Sotuv statistikasi")]
        ]
    elif role == 'omborchi':
        buttons = [
            [types.KeyboardButton(text="ЁЯФД Omborni yangilash"), types.KeyboardButton(text="ЁЯЪЪ Yetkazishlar nazorati")],
            [types.KeyboardButton(text="ЁЯУж Mavjud mebellar"), types.KeyboardButton(text="ЁЯУК Yetkazish hisoboti")]
        ]
    elif role == 'xodim':
        buttons = [
            [types.KeyboardButton(text="ЁЯФи Faol buyurtmalar")]
        ]
    else:
        buttons = [[types.KeyboardButton(text="ЁЯЫН Sotuvdagi mebellar")]]
    return types.ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)

# --- BOSH MENYU / BEKOR QILISH ---
MAIN_MENU_BUTTONS = {
    "Bosh menyu", "тЮХ Yangi mebel", "ЁЯУж Mavjud mebellar", 
    "ЁЯУЭ Yangi buyurtma", "ЁЯУЛ Buyurtmalar nazorati", "ЁЯУК Mijozlar hisoboti", "ЁЯЪЪ Haydovchilar hisoboti", "ЁЯХ░ Yetkazish tarixi", "ЁЯУИ Sotuv statistikasi",
    "ЁЯФД Omborni yangilash", "ЁЯЪЪ Yetkazishlar nazorati", "ЁЯУК Yetkazish hisoboti", "ЁЯФи Faol buyurtmalar", "ЁЯЫН Sotuvdagi mebellar"
}
...
@dp.message(F.text == "тЮХ Yangi mebel")
...
async def add_quantity(message: types.Message, state: FSMContext):
    ...
    await message.answer("ЁЯУ╖ Mahsulot rasmini yuboring (URL ko'rinishida):\nMasalan: https://example.com/rasm.jpg")
...
async def add_final(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"тЬЕ Mebel qo'shildi! ID: `{p_id}`")
    await message.answer_photo(photo=message.text, caption=f"ЁЯкС {data['name']} ({data['model']})\nЁЯТ░ Narxi: {data['price']} so'm\nЁЯУж Soni: {data['quantity']} ta")
    await state.clear()

# --- BARCHA UCHUN: OMBORNI KO'RISH ---
@dp.message(F.text.in_({"ЁЯУж Mavjud mebellar", "ЁЯЫН Sotuvdagi mebellar"}))
async def view_stock(message: types.Message):
    ...
    for m_id, m in mebellar.items():
        if isinstance(m, dict) and m.get('soni', 0) > 0:
            text = f"ЁЯкС **{m.get('nomi', 'Noma\'lum')}** ({m.get('modeli', '')})\n"
            if role == 'admin':
                text += f"ЁЯТ░ Narxi: {m.get('narxi', '0')}\n"
            text += f"ЁЯУж Qoldiq: {m.get('soni', 0)} ta\n"
            text += f"ЁЯЖФ ID: `{m_id}`"
            ...
            if rasm:
                try:
                    ...
                    await message.answer(text + f"\nЁЯУ╖ Rasmi: {rasm}", parse_mode="Markdown")
            else:
                await message.answer(text, parse_mode="Markdown")

# --- ADMIN: YANGI BUYURTMA YOZISH ---
@dp.message(F.text == "ЁЯУЭ Yangi buyurtma")
...
async def process_amount(message: types.Message, state: FSMContext):
    ...
    await message.answer("ЁЯУЕ Buyurtma qaysi sanaga tayyor bo'lishi kerak? Tugmalardan tanlang yoki yozing:", reply_markup=get_dates_keyboard())
...
async def process_due_date(message: types.Message, state: FSMContext):
    ...
    await message.answer("ЁЯУЭ Buyurtma uchun izoh kiriting (mebelning biror joyini o'zgartirish kerak bo'lsa):\n(Agar izoh bo'lmasa 'yoq' deb yozing)", reply_markup=types.ReplyKeyboardRemove())
...
async def process_comment(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"тЬЕ Buyurtma qabul qilindi!\nЁЯЖФ ID: {order_id}\nЁЯУЕ Muddat: {data['due_date']}\nЁЯУЭ Izoh: {comment}")
...
@dp.message(F.text == "ЁЯУК Mijozlar hisoboti")
...
async def show_client_report(message: types.Message, state: FSMContext):
    ...
    report_text = f"ЁЯСд **Mijoz:** {client_name}\n"
    report_text += f"ЁЯТ│ **Joriy qarzi:** {current_debt} so'm\n\n"
    report_text += "ЁЯУж **Olingan mebellar tarixi:**\n"
    ...
    if orders_ref:
        for o_id, o in orders_ref.items():
            if isinstance(o, dict) and str(o.get('client_name')).strip() == client_name.strip():
                count += 1
                report_text += f"тЦкя╕П {o.get('product_id')} - {o.get('amount')} ta ({o.get('status')})\n"
    ...
    markup = types.ReplyKeyboardMarkup(
        keyboard=[
            [types.KeyboardButton(text=f"тЮХ {client_name} dan Pul Olish (Kirim)")],
            [types.KeyboardButton(text=f"тЮЦ {client_name} ga Pul Berish (Chiqim)")],
            [types.KeyboardButton(text=f"ЁЯУЬ {client_name} To'lovlar Tarixi")],
...
async def client_kirim_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'admin':
        client_name = message.text.replace("тЮХ ", "").replace(" dan Pul Olish (Kirim)", "").strip()
        await state.update_data(client_name=client_name)
        await message.answer(f"ЁЯСд {client_name} dan olingan pul summasini kiriting (faqat raqam):", reply_markup=types.ReplyKeyboardRemove())
...
async def process_client_kirim(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"тЬЕ {client_name} dan {amount} so'm qabul qilindi.\nЁЯТ│ Yangi qarzi: {new_debt} so'm", reply_markup=main_menu('admin'))
        await state.clear()
    except ValueError:
...
async def client_chiqim_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'admin':
        client_name = message.text.replace("тЮЦ ", "").replace(" ga Pul Berish (Chiqim)", "").strip()
        await state.update_data(client_name=client_name)
        await message.answer(f"ЁЯСд {client_name} ga qarz sifatida yoziladigan yoki qaytariladigan pul summasini kiriting (faqat raqam):", reply_markup=types.ReplyKeyboardRemove())
...
async def process_client_chiqim(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"тЬЕ {client_name} qarziga {amount} so'm qo'shildi.\nЁЯТ│ Yangi qarzi: {new_debt} so'm", reply_markup=main_menu('admin'))
        await state.clear()
    except ValueError:
        await message.answer("Iltimos, faqat raqam kiriting:")
...
async def client_history(message: types.Message):
    if await get_user_role(message.from_user.id) == 'admin':
        client_name = message.text.replace("ЁЯУЬ ", "").replace(" To'lovlar Tarixi", "").strip()
        trans_ref = await asyncio.to_thread(db.reference(f'transactions/clients/{client_name}').get)
        if not trans_ref:
            await message.answer(f"ЁЯСд {client_name} bo'yicha to'lovlar tarixi yo'q.")
            return
            
        history = f"ЁЯУЬ **{client_name} to'lovlar (Kirim/Chiqim) tarixi:**\n\n"
        for t_id, t in trans_ref.items():
            if isinstance(t, dict):
                icon = "ЁЯЯв" if t.get('type') == 'Kirim' else "ЁЯФ┤"
                history += f"{icon} {t.get('type')}: {t.get('amount')} so'm\n"
                history += f"ЁЯУЕ Sana: {t.get('timestamp')}\n\n"
...
async def notify_warehouse(order_data, order_id):
    ...
    try:
        await bot.send_message(
            883589794,
            f"ЁЯФФ Yangi buyurtma!\n\n"
            f"ЁЯзС Mijoz: {order_data['client']}\n"
            f"ЁЯУж Mebel: {order_data['product_id']}\n"
            f"ЁЯУК Soni: {order_data['amount']}\n"
            f"ЁЯУЕ Muddat: {order_data['due_date']}\n"
            f"ЁЯУЭ Izoh: {order_data.get('comment', 'Yoq')}\n"
            f"ЁЯЖФ Buyurtma ID: {order_id}"
        )
...
@dp.message(F.text == "ЁЯФи Faol buyurtmalar")
async def view_active_orders(message: types.Message):
    ...
    if role in ['xodim', 'admin', 'omborchi']:
        ...
        for o_id, o in active_orders_list:
            active_orders += f"ЁЯЖФ `{o_id}` - ЁЯзС Mijoz: {o.get('client_name')}\n"
            active_orders += f"ЁЯУж Mebel: {o.get('product_id')}\n"
            active_orders += f"ЁЯУК Soni: {o.get('amount')} ta\n"
            active_orders += f"ЁЯУЕ Muddat: {o.get('due_date')}\n"
            if o.get('comment') and str(o.get('comment')).lower() != 'yoq':
                active_orders += f"ЁЯУЭ Izoh: {o.get('comment')}\n"
            active_orders += "\n"
        ...
        await message.answer(f"ЁЯФи Faol buyurtmalar ro'yxati:\n\n{active_orders}")

# --- OMBORCHI: YETKAZISH HISOBOTI ---
@dp.message(F.text == "ЁЯУК Yetkazish hisoboti")
async def warehouse_delivery_report(message: types.Message):
    ...
    if role == 'omborchi':
        ...
        report_text = f"ЁЯУК **{current_month} oyi uchun yetkazib berish hisoboti:**\n\n"
        ...
        if deliveries_ref:
            ...
            for d_id, d in items:
                if isinstance(d, dict):
                    ...
                    report_text += f"ЁЯЖФ ID: `{order_id}`\n"
                    report_text += f"ЁЯзС Mijoz: {client}\n"
                    report_text += f"ЁЯУж Mebel: {product}\n"
                    report_text += f"ЁЯУЕ Buyurtma sanasi: {order_date}\n"
                    report_text += f"ЁЯЪЪ Yetkazilgan sana: {delivery_date}\n"
                    report_text += f"ЁЯСитАНтЬИя╕П Haydovchi: {driver}\n"
                    report_text += "------------------------\n"
...
@dp.message(F.text == "ЁЯФД Omborni yangilash")
...
async def update_stock_new_quantity(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"тЬЕ Ombor muvaffaqiyatli yangilandi!\nYangi qoldiq: {new_quantity} ta", reply_markup=main_menu('omborchi'))
        await state.clear()
    except ValueError:
        await message.answer("Iltimos, faqat raqam kiriting:")

# --- OMBORCHI: YETKAZIB BERISH NAZORATI ---
@dp.message(F.text == "ЁЯЪЪ Yetkazishlar nazorati")
async def delivery_control_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'omborchi':
        ...
        row = []
        for o_id, o in active_orders_list:
            active_orders += f"ЁЯЖФ `{o_id}` - ЁЯзС {o.get('client_name')}\nЁЯУж Mebel: {o.get('product_id')} ({o.get('amount')} ta)\nЁЯУЕ Muddat: {o.get('due_date')}\n"
            if o.get('comment') and str(o.get('comment')).lower() != 'yoq':
                active_orders += f"ЁЯУЭ Izoh: {o.get('comment')}\n"
            active_orders += f"ЁЯУМ Holati: {o.get('status')}\n\n"
...
async def delivery_new_status(message: types.Message, state: FSMContext):
    ...
    if new_status in ["Mijozni o'zi olib ketdi", "Bekor qilindi", "Tayyor bo'ldi"]:
        ...
        await message.answer(f"тЬЕ Buyurtma holati yangilandi: {new_status}", reply_markup=main_menu('omborchi'))
        ...
        for user_id, user_data in (users_ref or {}).items():
            if user_data.get('role') == 'admin':
                try:
                    await bot.send_message(
                        int(user_id),
                        f"ЁЯУж **Buyurtma holati o'zgardi!**\n\nЁЯЖФ ID: `{data['order_id']}`\nЁЯзС Mijoz: {data['client']}\nЁЯУМ Yangi holat: {new_status}",
                        parse_mode="Markdown"
                    )
                except Exception:
                    pass
                ...
                try:
                    await bot.send_message(
                        int(user_id),
                        f"тЬЕ **Mahsulot tayyor bo'ldi!**\n\nЁЯЖФ Buyurtma ID: `{data['order_id']}`\nЁЯзС Mijoz: {data['client']}\nЁЯУж Mebel: {product_id}\nЁЯУК Soni: {amount} ta",
                        parse_mode="Markdown"
                    )
                except Exception:
                    pass
...
async def process_delivery_final(price, message: types.Message, state: FSMContext):
    ...
    await message.answer(f"тЬЕ Buyurtma holati yangilandi: {new_status}\nЁЯЪЪ Haydovchi: {driver}\nЁЯТ╡ Narxi: {price}", reply_markup=main_menu('omborchi'))
...
@dp.message(F.text == "ЁЯЪЪ Haydovchilar hisoboti")
async def driver_report_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'admin':
        ...
        driver_stats = {}
        report_text = f"ЁЯУК **{current_month} oyi uchun haydovchilar hisoboti:**\n\n"
        ...
        for driver_name, stats in driver_stats.items():
            report_text += f"ЁЯЪЪ **{driver_name}**\n"
            report_text += f"ЁЯУж Jami yetkazib berishlar: {stats['count']} ta\n"
            ...
            report_text += f"ЁЯТ░ Narxlar: {', '.join(stats['total_price'])} (Jami: {sum_text})\n"
            report_text += f"ЁЯСе Mijozlar: {', '.join(stats['clients'])}\n\n"
        ...
        for i in range(0, len(drivers), 2):
            row = [types.KeyboardButton(text=f"ЁЯСитАНтЬИя╕П {drivers[i]}")]
            if i + 1 < len(drivers):
                row.append(types.KeyboardButton(text=f"ЁЯСитАНтЬИя╕П {drivers[i+1]}"))
            buttons.append(row)
        buttons.append([types.KeyboardButton(text="Bosh menyu")])
        markup = types.ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)
...
@dp.message(F.text.startswith("ЁЯСитАНтЬИя╕П "))
async def select_driver_finance(message: types.Message):
    if await get_user_role(message.from_user.id) == 'admin':
        driver_name = message.text.replace("ЁЯСитАНтЬИя╕П ", "").strip()
        ...
        markup = types.ReplyKeyboardMarkup(
            keyboard=[
                [types.KeyboardButton(text=f"тЮХ {driver_name} ga Pul Berish (D. Chiqim)")],
                [types.KeyboardButton(text=f"тЮЦ {driver_name} dan Pul Qaytardi (D. Kirim)")],
                [types.KeyboardButton(text=f"ЁЯУЬ {driver_name} Moliya Tarixi")],
                [types.KeyboardButton(text=f"ЁЯУК {driver_name} yetkazib berish tarixi")],
                [types.KeyboardButton(text="Bosh menyu")]
            ],
            resize_keyboard=True
        )
        
        text = f"ЁЯСитАНтЬИя╕П **Haydovchi:** {driver_name}\n"
        text += f"ЁЯТ│ **Joriy balansi (sizning qarzingiz/haqqingiz):** {current_bal} so'm\n\n"
...
async def driver_chiqim_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'admin':
        driver_name = message.text.replace("тЮХ ", "").replace(" ga Pul Berish (D. Chiqim)", "").strip()
        await state.update_data(driver_name=driver_name)
        await message.answer(f"ЁЯСитАНтЬИя╕П {driver_name} ga qancha pul berildi? (faqat raqam):", reply_markup=types.ReplyKeyboardRemove())
...
async def process_driver_chiqim(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"тЬЕ {driver_name} ga {amount} so'm berildi.\nЁЯТ│ Yangi balansi: {new_bal} so'm", reply_markup=main_menu('admin'))
        await state.clear()
    except ValueError:
        await message.answer("Iltimos, faqat raqam kiriting:")
...
async def driver_kirim_start(message: types.Message, state: FSMContext):
    if await get_user_role(message.from_user.id) == 'admin':
        driver_name = message.text.replace("тЮЦ ", "").replace(" dan Pul Qaytardi (D. Kirim)", "").strip()
        await state.update_data(driver_name=driver_name)
        await message.answer(f"ЁЯСитАНтЬИя╕П {driver_name} qancha pul qaytardi yoki haqqi qo'shildi? (faqat raqam):", reply_markup=types.ReplyKeyboardRemove())
...
async def process_driver_kirim(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"тЬЕ {driver_name} hisobiga {amount} so'm qo'shildi.\nЁЯТ│ Yangi balansi: {new_bal} so'm", reply_markup=main_menu('admin'))
        await state.clear()
    except ValueError:
        await message.answer("Iltimos, faqat raqam kiriting:")
...
async def driver_history(message: types.Message):
    if await get_user_role(message.from_user.id) == 'admin':
        driver_name = message.text.replace("ЁЯУЬ ", "").replace(" Moliya Tarixi", "").strip()
        trans_ref = await asyncio.to_thread(db.reference(f'transactions/drivers/{driver_name}').get)
        if not trans_ref:
            await message.answer(f"ЁЯСитАНтЬИя╕П {driver_name} bo'yicha moliya (Kirim/Chiqim) tarixi yo'q.")
            return
            
        history = f"ЁЯУЬ **{driver_name} moliya tarixi:**\n\n"
        for t_id, t in trans_ref.items():
            if isinstance(t, dict):
                icon = "ЁЯФ┤" if t.get('type') == 'Chiqim' else "ЁЯЯв"
                history += f"{icon} {t.get('type')}: {t.get('amount')} so'm\n"
                history += f"ЁЯУЕ Sana: {t.get('timestamp')}\n\n"
...
@dp.message(F.text == "ЁЯХ░ Yetkazish tarixi")
...
async def show_delivery_history(message: types.Message, state: FSMContext):
    ...
    history_text = f"ЁЯХ░ **{month} oyi yetkazib berish tarixi:**\n\n"
    for d_id, d in deliveries_ref.items():
        if isinstance(d, dict):
            history_text += f"ЁЯУЕ Vaqt: {d.get('timestamp', 'Noma\'lum')}\n"
            history_text += f"ЁЯзС Mijoz: {d.get('client', 'Noma\'lum')}\n"
            history_text += f"ЁЯУж Mebel: {d.get('product_id', 'Noma\'lum')}\n"
            history_text += f"ЁЯЪЪ Dostavchik: {d.get('driver', 'Noma\'lum')} ({d.get('price', '0')})\n\n"
...
@dp.message(F.text == "ЁЯУИ Sotuv statistikasi")
async def sales_statistics(message: types.Message):
    if await get_user_role(message.from_user.id) == 'admin':
        ...
        text = "ЁЯУИ **Eng ko'p sotilgan mebellar (Oylar kesimida):**\n\n"
        
        for month in sorted_months:
            text += f"ЁЯУЕ **{month} oyi:**\n"
...
async def driver_deliveries_history(message: types.Message):
    if await get_user_role(message.from_user.id) == 'admin':
        driver_name = message.text.replace("ЁЯУК ", "").replace(" yetkazib berish tarixi", "").strip()
        ...
        if not monthly_stats:
            await message.answer(f"ЁЯСитАНтЬИя╕П {driver_name} hech qanday mebel yetkazib bermagan.")
            return
            
        sorted_months = sorted(monthly_stats.keys(), reverse=True)
        report_text = f"ЁЯУК **{driver_name} yetkazib berish tarixi:**\n\n"
        
        for month in sorted_months:
            month_deliveries = monthly_stats[month]
            report_text += f"ЁЯУЕ **{month} oyi:**\n"
            count = 0
            for d in month_deliveries:
                count += 1
                report_text += f" тЦкя╕П {d.get('client')} ga: {d.get('product_id')} (Narxi: {d.get('price')})\n"
            report_text += f" ЁЯУж Jami shu oyda: {count} ta yetkazib berish\n\n"
...
@dp.message(F.text == "ЁЯУЛ Buyurtmalar nazorati")
async def admin_order_control_start(message: types.Message, state: FSMContext):
    ...
    for o_id, o in active_orders_list:
        active_orders += f"ЁЯЖФ `{o_id}` - ЁЯзС {o.get('client_name')}\n"
        active_orders += f"ЁЯУж Mebel: {o.get('product_id')} ({o.get('amount')} ta)\n"
        active_orders += f"ЁЯУЕ Muddat: {o.get('due_date')}\n"
        if o.get('comment') and str(o.get('comment')).lower() != 'yoq':
            active_orders += f"ЁЯУЭ Izoh: {o.get('comment')}\n"
        active_orders += f"ЁЯУМ Holati: {o.get('status')}\n\n"
...
async def admin_order_selected(message: types.Message, state: FSMContext):
    ...
    info = f"ЁЯЖФ Buyurtma: `{order_id}`\n"
    info += f"ЁЯзС Mijoz: {order_ref.get('client_name')}\n"
    info += f"ЁЯУж Mebel: {order_ref.get('product_id')}\n"
    info += f"ЁЯУК Soni: {order_ref.get('amount')} ta\n"
    info += f"ЁЯУЕ Muddat: {order_ref.get('due_date')}\n"
    if order_ref.get('comment') and str(order_ref.get('comment')).lower() != 'yoq':
        info += f"ЁЯУЭ Izoh: {order_ref.get('comment')}\n"
    info += f"ЁЯУМ Holati: {order_ref.get('status')}\n"
    
    markup = types.ReplyKeyboardMarkup(
        keyboard=[
            [types.KeyboardButton(text="тЭМ Bekor qilish"), types.KeyboardButton(text="тЬПя╕П O'zgartirish")],
            [types.KeyboardButton(text="Bosh menyu")]
        ],
        resize_keyboard=True
    )
...
async def admin_order_action(message: types.Message, state: FSMContext):
    ...
    if message.text == "тЭМ Bekor qilish":
        order_ref = await asyncio.to_thread(db.reference(f'orders/{order_id}').get)
        if order_ref:
            # Omborga qaytarish
        ...
        await message.answer(f"тЬЕ Buyurtma `{order_id}` bekor qilindi!\nЁЯУж Omborga qaytarildi.", reply_markup=main_menu('admin'), parse_mode="Markdown")
        ...
        for user_id, user_data in (users_ref or {}).items():
            if user_data.get('role') in ['omborchi', 'ishchi', 'xodim']:
                try:
                    await bot.send_message(
                        int(user_id),
                        f"тЪая╕П Buyurtma bekor qilindi!\nЁЯЖФ ID: {order_id}\nЁЯУж Mebel: {order_ref.get('product_id')}\nЁЯзС Mijoz: {order_ref.get('client_name')}"
                    )
                except:
                    pass
        # Hardcoded omborchi
        try:
            await bot.send_message(883589794, f"тЪая╕П Buyurtma bekor qilindi!\nЁЯЖФ ID: {order_id}\nЁЯУж Mebel: {order_ref.get('product_id')}\nЁЯзС Mijoz: {order_ref.get('client_name')}")
        except:
            pass
    ...
    elif message.text == "тЬПя╕П O'zgartirish":
        markup = types.ReplyKeyboardMarkup(
            keyboard=[
                [types.KeyboardButton(text="ЁЯУК Sonini o'zgartirish"), types.KeyboardButton(text="ЁЯУЕ Muddatni o'zgartirish")],
                [types.KeyboardButton(text="ЁЯУЭ Izohni o'zgartirish")],
                [types.KeyboardButton(text="Bosh menyu")]
            ],
            resize_keyboard=True
        )
...
async def admin_order_edit_field(message: types.Message, state: FSMContext):
    ...
    field_map = {
        "ЁЯУК Sonini o'zgartirish": "amount",
        "ЁЯУЕ Muddatni o'zgartirish": "due_date",
        "ЁЯУЭ Izohni o'zgartirish": "comment"
    }
...
async def admin_order_new_value(message: types.Message, state: FSMContext):
    ...
    if field == "amount":
        ...
        await message.answer(f"тЬЕ Buyurtma `{order_id}` soni {new_amount} taga o'zgartirildi!", reply_markup=main_menu('admin'), parse_mode="Markdown")
    
    elif field == "due_date":
        await asyncio.to_thread(db.reference(f'orders/{order_id}').update, {'due_date': new_value})
        await message.answer(f"тЬЕ Buyurtma `{order_id}` muddati {new_value} ga o'zgartirildi!", reply_markup=main_menu('admin'), parse_mode="Markdown")
    
    elif field == "comment":
        await asyncio.to_thread(db.reference(f'orders/{order_id}').update, {'comment': new_value})
        await message.answer(f"тЬЕ Buyurtma `{order_id}` izohi o'zgartirildi!", reply_markup=main_menu('admin'), parse_mode="Markdown")
    ...
    for user_id, user_data in (users_ref or {}).items():
        if user_data.get('role') in ['omborchi', 'ishchi', 'xodim']:
            try:
                await bot.send_message(
                    int(user_id),
                    f"тЬПя╕П Buyurtma o'zgartirildi!\nЁЯЖФ ID: {order_id}\nЁЯУж {order_ref.get('product_id')}\nЁЯзС Mijoz: {order_ref.get('client_name')}\nЁЯФД {field_names.get(field, field)}: {new_value}"
                )
            except:
                pass
    try:
        await bot.send_message(883589794, f"тЬПя╕П Buyurtma o'zgartirildi!\nЁЯЖФ ID: {order_id}\nЁЯУж {order_ref.get('product_id')}\nЁЯзС Mijoz: {order_ref.get('client_name')}\nЁЯФД {field_names.get(field, field)}: {new_value}")
    except:
        pass
